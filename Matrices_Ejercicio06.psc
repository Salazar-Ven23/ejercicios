...
Algoritmo Matrices_Ejercicio06
	
	Definir MagicMatriz, order, i, j, k Como Entero
	
	Escribir "Elija el tamaño de la matriz (no superior a 10)";
	Leer order;
	
	Dimension MagicMatriz[order, order]
	...
	Para i = 0 Hasta order - 1
		Para j = 0 Hasta order - 1
			...
			Repetir
				...
				Leer MagicMatriz[i, j]
			Mientras Que MagicMatriz[i, j] < 1 | MagicMatriz[i, j] > 9
			showProgress(MagicMatriz, i, k, j)
			Si k < order - 1
				k = k + 1
			FinSi
		FinPara
	...
	validateArray(MagicMatriz, order)
